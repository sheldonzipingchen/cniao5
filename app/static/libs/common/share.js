define(function(require, exports, module) {
    ...
    $('body').on('click', '.js-social-share', function() {
        ...
        if(params==undefined)
            params = $("body").find('.js-social-share-params').data()
    ...
    function weixin($btn, params)
    {
        if ($('.weixin-share-modal').length == 0) {
            ...
            $modal.on('show.bs.modal', function(){
                ...
                var url = params.url+"?sns="+params.s+"&channel="+params.inviteCode;
                $modal.find('.weixin-share-qrcode').qrcode(url);
...
//        html += '        <p class="weixin-share-loading" style="text-align:center;">正在加载二维码...</p>';
    ...
    function weibo(params)
    {
        var query = {};
        query.s='weibo';
        query.url = params.url+"?sns="+query.s+"&channel="+params.inviteCode;
    ...
    function makeQRcode(text){
        ...
        var qr = new QRCode('.weixin-share-qrcode',{
            text:text
        });
